...
def build_readable_markdown(
    ...
    front_matter_lines = [
        ...
        f"title: \"{title or ''}\"",
        f"input_url: {input_url or ''}",
        f"final_url: {final_url or ''}",
        f"fetched_at: {fetched_at or ''}",
    ...
    body = apply_redactions(content_md or "", redact_patterns).strip()

    # Optional extracted links table
    links_section = ""
    if include_links_table and extracted_links:
        table_lines = [
            "### Extracted Links",
            "",
            "| Text | URL |",
        ...
        links_section = "\n\n" + "\n".join(table_lines)
        links_section = apply_redactions(links_section, redact_patterns)
...
def build_combined_markdown(manifest: Manifest, source_markdowns: Dict[str, str]) -> str:
    ...
    doc_lines = [
        ...
        f"- **Tool:** {manifest.tool.name} v{manifest.tool.version}",
        "",
        "## Usage",
